...
def column_help(keyword_width=10, total_width=70):
    ...
    for entry in PS_FIELD_HELP:
        ...
        help.append("%s%s" % (entry[0].ljust(keyword_width), desc))
    return "\n".join(help)
...
def poll_process(
    ...
    ps = subprocess.Popen(ps_invocation, shell=True, stdout=subprocess.PIPE)
    ...
    for row in stdout.split("\n"):
        ...
        if len(fields) != 8:
            ...
            sys.stderr.write(
                "SYRUPY: Skipping sample: found only %d columns: %s"
                % (len(fields), fields)
            )
            continue
        if (
            (not ignore_self or int(fields[0]) != os.getpid())
...
def profile_process(
    ...
    while not quit:
        ...
        if top_mem is not None:
            pinfoset = sorted(pinfoset, key=lambda v: int(v["vsz"]), reverse=True)[
                :top_mem
            ]
...
standard output and standard error ('-C'), or suppress all COMMAND output altogether ('-N').
    ...
    if opts.syrupy_in_front:
        ...
        if not opts.quiet:
            sys.stderr.write(
                "SYRUPY: Writing process resource usage samples to '%s'\n" % fname
            )
    ...
    if opts.suppress_raw_process_log:
        ...
        if not opts.quiet:
            sys.stderr.write(
                "SYRUPY: Writing raw process resource usage logs to '%s'\n" % fname
            )
        raw_ps_log = open_file(base_title + ".ps.raw", "w", replace=opts.replace)

    if (
        opts.poll_pid is not None
        or opts.poll_command is not None
        or opts.poll_mem is not None
    ):
        if not opts.quiet:
            if opts.poll_pid is not None:
                ...
                sys.stderr.write(
                    "SYRUPY: sampling top %d processes by memory usage\n"
                    % opts.poll_mem
                )
            else:
                sys.stderr.write(
                    "SYRUPY: sampling process with command pattern '%s'\n"
                    % opts.poll_command
                )
        profile_process(
            pid=opts.poll_pid,
        ...
        if opts.suppress_command_output:
            ...
            if not opts.quiet:
                sys.stderr.write(
                    "SYRUPY: Redirecting command output stream to '%s'\n" % cout
                )
            command_stdout = open_file(cout, "w", replace=opts.replace)
            if not opts.quiet:
                sys.stderr.write(
                    "SYRUPY: Redirecting command error stream to '%s'\n" % cerr
                )
            command_stderr = open_file(cerr, "w", replace=opts.replace)
        start_time, end_time = profile_command(
            command=command,
        ...
        if not opts.quiet:
            final_run_report = []
            final_run_report.append(
                "SYRUPY: Completed running: %s" % (" ".join(command))
            )
            final_run_report.append(
                "SYRUPY: Started at %s" % (start_time.isoformat(" "))
            )
            final_run_report.append("SYRUPY: Ended at %s" % (end_time.isoformat(" ")))
            hours, mins, secs = str(end_time - start_time).split(":")
            run_time = (
                "SYRUPY: Total run time: %s hour(s), %s minute(s), %s second(s)"
                % (hours, mins, secs)
            )
